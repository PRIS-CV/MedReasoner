...
def seg_point_l1_reward(predict_str: str, ground_truth: str) -> float:
    ...
    try: 
        gt_points = ground_truth.get("point_2d", [])
        json_pattern = r'{[^}]+}' 
        json_match = re.search(json_pattern, predict_str)
...
def compute_score(predict_str: str, ground_truth: str):
    ...
    rewards = {
        "thinking": seg_thinking_format_reward(predict_str),
        "segment": seg_segmentation_format_reward(predict_str),
        "iou": seg_iou_reward(predict_str, gt),
        "bbox_l1": seg_box_l1_reward(predict_str, gt),
        "points_l1": seg_point_l1_reward(predict_str, gt),
    }
