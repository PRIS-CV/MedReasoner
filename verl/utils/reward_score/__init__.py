...
def default_compute_score(data_source, solution_str, ground_truth, extra_info=None, sandbox_fusion_url=None, concurrent_semaphore=None, memory_limit_mb=None):
    ...
    elif data_source == "umrg":
        reward_mode = 'soft' # Default to 'soft', can be set to ['soft', 'hard', 'base'] as needed
        if reward_mode == "base":
            from . import med_base
            
            res = med_base.compute_score(solution_str, ground_truth)
        elif reward_mode in ['hard', 'soft']:
            from . import med_grounding
            
            res = med_grounding.compute_score(solution_str, ground_truth, reward_mode)
    else:
